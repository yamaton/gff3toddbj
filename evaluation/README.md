...
アノテーションファイルの各行から（エントリ名、feature名、補正なしの位置情報）を要素とする多重集合を作ります。この位置情報にて開始点・終了点にコドンが無いことを示す不等号の存在は無視します。たとえば `join(12..78,134..202)` と `join(<12..78,134..202)`は同じものとみなします。


#### 2. 位置情報（補正等すべて含む）

１と同様に（エントリ名、feature名、location情報）を要素とする多重集合をつくります。が、２ではlocation情報には開始・終了点に付けられる不等号も考慮します。つまり`join(12..78,134..202)` と `join(<12..78,134..202)`は別物として扱います。


### 3. Feature-Qualifier 情報

アノテーションファイルの各行から（エントリ名、Feature名、Qualifierキー名）を要素とする多重集合を作ります。
...
* RefSeq仕様との差がいくつか判明
    ...
    * `/pseudo`の扱いの違い
        * DDBJでは[新規登録で記載しない方針](https://www.ddbj.nig.ac.jp/ddbj/qualifiers.html#pseudo)のため、`/pseudogene="unknown"` に置き換えている
    * `assembly_gap` の表記の省略
        * フラットファイルにて `assembly_gap` が付けられる基準が不明瞭
